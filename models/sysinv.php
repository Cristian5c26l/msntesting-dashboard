...
class m_sysinv{
    ...
    public function selectMarksByInsumeType($host,$user,$pass,$db, $insTypeId){

        $con = $this->connect($host,$user,$pass,$db);

        $sql = "SELECT a.id_mark AS id, b.mark AS mark FROM adm_marcas_sis_por_tipo_insumo a 
        
        INNER JOIN adm_marcas_sis b ON b.id = a.id_mark
        
        WHERE a.id_insume=$insTypeId AND a.status=1";
