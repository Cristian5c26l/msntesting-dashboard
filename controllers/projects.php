...
class c_project{
    ...
    public function selectUserProjectsL($idarea=null){
    ...
    public function selectUserProjectsG($idarea=null){
    ...
    public function selectTask($id,$type=null){
        ...
        if($type==1){
            ...
            $comment = $row['comment'];
                ...
                ?>
                    <form class="row" onsubmit="editTaskPro(<?php echo $id; ?>); return false;">
                    ...
                    <td>
                        <div class="table-buttons">
                            ...
                            <div class="dropdown-menu">
                                <div onclick="editStatusTaskPro('<?php echo $id; ?>',1);" class="dropdown-item">Finalizar</div>
                                <div onclick="editStatusTaskPro('<?php echo $id; ?>',2);" class="dropdown-item">Reabrir</div>
                                <div onclick="editStatusTaskPro('<?php echo $id; ?>',7);" class="dropdown-item">Fix/Detenido</div>
                            </div>
                            ...
                            <button onclick="editableTaskPro('<?php echo $id; ?>')" type="button" class="btn btn-outline-primary btn-sm" <?php echo $disabled; ?>>
                                <i class="material-icons md-14 align-middle mb-1 text-primary">edit</i>
                            </button>    
                            <button onclick="delTaskPro('<?php echo $id; ?>')" type="button" class="btn btn-outline-danger btn-sm" <?php echo $disabled; ?>>
                                <i class="material-icons md-14 align-middle mb-1 text-danger">delete</i>
                            </button>
                            <?php
                            }
    ...
    public function selectAllStatus($idTask){
        ...
        while($row = mysqli_fetch_array($res)){
            ...
            ?>
                <div id="<?php echo $id; ?>" onclick="assignPriorityTaskPro('<?php echo $idTask; ?>','<?php echo $id; ?>','<?php echo $priority; ?>');" class="dropdown-item"><?php echo $priority; ?></div>
            <?php
        }
    }
    ...
    public function employee($area,$idTask){
        ...
        while($row = mysqli_fetch_array($res)){
            ...
            ?>
                <div id="<?php echo $id; ?>" onclick="assignTaskPro('<?php echo $idTask; ?>','<?php echo $id; ?>','<?php echo $employe; ?>');" class="dropdown-item"><?php echo $employe; ?></div>
            <?php
        }
    }
    ...
    public function selectProjectsJson($idarea=NULL){
        
        $res = $this->model->selectProject($this->host,$this->user,$this->pass,$this->db,$idarea);
        ...
        while($row = mysqli_fetch_array($res)){
            ...
            /*$array[$sum]['TaskID'] = $id; 
            $array[$sum]['area'] = $area;
            $array[$sum]['TaskName'] = $project;
            $array[$sum]['description'] = $descript;
            $array[$sum]['StartDate'] = $date_ini;
            $array[$sum]['EndDate'] = $date_end;
            $array[$sum]['percent'] = $percent;
            $array[$sum]['stats'] = $stats;*/

            $res1 = $this->model->selectUserTask($this->host,$this->user,$this->pass,$this->db,$id,$iduser=null);
            ...
            while($row1 = mysqli_fetch_array($res1)){
                $idtask = $row1['id'];
                $title = $row1['title'];
                $dateini = $row1['date_ini'];
                $dateend = $row1['date_end'];

                ?>
                        { TaskID: <?php echo $idtask; ?>, TaskName: '<?php echo $title; ?>', StartDate: new Date('<?php echo $dateini; ?>'), Duration: 4, Progress: <?php echo $percent; ?> },
                <?php

                /*$array[$sum]['subtasks'][$sum2]['TaskID'] = $idtask;
                $array[$sum]['subtasks'][$sum2]['TaskName'] = $title;
                $array[$sum]['subtasks'][$sum2]['StartDate'] = $dateini;*/
